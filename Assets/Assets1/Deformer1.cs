...
public class Deformer1 : MonoBehaviour
{
    ...
    { // 각 변수 초기화
        ...
        meshcollider = GetComponent<MeshCollider>();                //콜라이더
                                                                    //rad = displacedVertices[displacedVertices.Length - 1].magnitude;
                                                                    //마지막 버텍스의 길이계산
        rad = displacedVertices[0].magnitude*1.2f;

        //비눗방울의 중심점 계산
        center = rb.centerOfMass;                               //..
                                                                //centerofmass는 무게중심인데 이걸 변경하면 오브젝트의 gizmo가 변경되고 상대좌표가 (0,0,0)으로 바뀜
    }
    ...
    {                                 // 점의 이동을 담당하는 함수
        ...
        float rd = Mathf.Abs(r-rad);                          //처음 모양에서 멀리 이동할수록(움직임의 폭이 클수록) 감속하기 위한 변수
        ...
        //Debug.Log(rd);
        velocity -= acel * displacement * Time.deltaTime/100;   //가속도를 속도로 바꿈 
        //velocity *= 1f - damping * Time.deltaTime;
        velocity *= (1f - (Time.deltaTime*2));                        //시간이 감에 따라 출렁이는 속도 감소
    ...
    { //힘으로 부터 속도를 구하는 함수
        ...
        float velocity = attenuatedForce * Time.deltaTime*2;      //속도 값 계산
